...
from libact.query_strategies import UncertaintySampling, RandomSampling, QUIRE, QueryByCommittee, HintSVM
...
def simulate_w2v(tweet_id):
    file = openfile_txt(dataset_filepath)
    for line in file:
        parts = line.replace('\n', '').replace('    ', ' ').replace('  ', ' ').split(";")
        if tweet_id in parts[1]:
            vectorized = parts[0]
            break
    return vectorized
...
def main():
    ...
    qs2 = UncertaintySampling(trn_ds2, method='sm', model=LogisticRegression(C=.01))
    model.train(trn_ds2)
    # preds = model.predict(tst_ds.format_sklearn()[0])
    # score = accuracy_score(tst_ds.format_sklearn()[1], preds)
    ...
    qs3 = UncertaintySampling(trn_ds3, method='entropy', model=LogisticRegression(C=.01))
    model.train(trn_ds3)
    # preds = model.predict(tst_ds.format_sklearn()[0])
    # score = accuracy_score(tst_ds.format_sklearn()[1], preds)
    ...
    plt.legend(('Least Confident', 'Smallest Margin', 'Entropy', 'Random Sampling', 'QUIRE', 'Vote Entropy', 'KL Divergence'), loc=1)
    ...
    for i in range(quota):
        ...
        ax.set_xlim((0, i + 1))
        ax.set_ylim((0, max(max(E_out1), max(E_out2), max(E_out3), max(E_out4), max(E_out5), max(E_out6), max(E_out7)) + 0.2))
